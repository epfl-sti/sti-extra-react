...
const getContinents = () => Array.from(new Set(countries.map(x => x.continent)))

const getDataByContinent = (lang) =>
  getContinents().reduce((obj, continent) => {
    obj[continent] = {
      population: countries.filter(x => x.continent === continent && x.languagecodes.includes(lang)).map(x => x.population),
      area: countries.filter(x => x.continent === continent && x.languagecodes.includes(lang)).map(x => x.area)
    }
    return obj
  }, {})
